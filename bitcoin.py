...
def computeDelta(wt, X, Xi):
    ...
    def computeSimilarity(X, Xi, wt):
        ...
        for i in range(0, len(X) - 1):
                simi += (X[i] - meanX) * (Xi[i] - meanXi)

        sim = np.exp(wt * simi / ((len(X) - 1) * stdX * stdXi))     
        return sim

    simi, simj = 0, 0    
    for i in range(0, len(Xi)):
        simi += Xi.iloc[i][-1] * computeSimilarity(X, Xi.iloc[i], wt)

    for i in range(0, len(Xi)):
        simj += computeSimilarity(X, Xi.iloc[i], wt)
...
# YOUR CODE HERE
linmodel = smf.ols(formula = 'deltaP ~ deltaP90 + deltaP180 +deltaP360', data = trainData)
model = linmodel.fit()
